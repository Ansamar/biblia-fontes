...
export default async function DynamicChapterPage({ params }: { params: Promise<{ libro: string; capitolo: string }> }) {
  ...
  const biblicalText = sanityTexts[0] || fixture;
  const biblicalTexts = sanityTexts.length ? sanityTexts : (fixture ? [fixture] : []);
  ...
  return <AppShell><main className="mx-auto max-w-[1120px] px-5 py-10 md:px-8 md:py-14">
    ...
    <UniversalChapterStudy chapter={chapter} reference={reference} worldNarratedLabel={chapter.eventiNarrati || libro.mondoDelTesto} biblicalText={biblicalText} biblicalTexts={biblicalTexts} />
  </main></AppShell>;
}
