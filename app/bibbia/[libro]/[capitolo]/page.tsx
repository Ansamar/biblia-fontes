...
export default async function DynamicChapterPage({params,searchParams}:{params:Promise<{libro:string;capitolo:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const libro=data.libro;const chapter=enrichGenesis(slug,data.capitolo,numero);const total=libro.capitoli||numero;if(numero>total)notFound();const abbr=bookAbbreviation(slug,libro.titolo);const reference=`${abbr} ${numero}`;const category=categoryLabel(libro.categoriaId);
