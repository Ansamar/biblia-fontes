...
export default async function DynamicBookPage({ params }: { params: Promise<{ libro: string }> }) {
  ...
  return <AppShell><main>
    <section className="border-b border-papyrus-line bg-paper-card/35"><div className="mx-auto max-w-[1180px] px-5 py-10 md:px-8 md:py-14">
    ...
    <section className="mx-auto max-w-[1180px] px-5 py-12 md:px-8 md:py-16"><div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_320px]"><div><div className="mb-6 flex items-end justify-between"><div><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Navigazione</p><h2 className="mt-2 font-serif text-3xl font-bold">Capitoli</h2></div><span className="text-sm text-ink-faint">{capitoli.length} disponibili</span></div><div className="divide-y divide-papyrus-line border-y border-papyrus-line">{capitoli.map((c:any) => <Link key={c._id} href={`/bibbia/${slug}/${c.numero}`} className="group grid min-h-16 grid-cols-[3.4rem_1fr_auto] items-center gap-3 py-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-bronze"><span className="font-mono text-xs text-ink-faint">{String(c.numero).padStart(2,'0')}</span><span className="font-serif text-xl font-semibold group-hover:text-bronze">{c.titolo || `Capitolo ${c.numero}`}</span><span className="text-xs text-ink-faint">{abbr} {c.numero} →</span></Link>)}</div>{!capitoli.length && <p className="mt-5 rounded-xl border border-papyrus-line bg-paper-card/50 p-5 text-ink-soft">Nessun capitolo collegato a questo libro nel dataset corrente.</p>}</div>
      <aside className="h-fit space-y-5 lg:sticky lg:top-24"><div className="rounded-2xl border border-papyrus-line bg-paper-card p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">{lens.title}</p><p className="mt-3 text-sm leading-6 text-ink-soft">{lens.intro}</p><ul className="mt-5 space-y-2 text-sm text-ink">{lens.items.map((item) => <li key={item} className="flex gap-2"><span className="text-bronze" aria-hidden="true">—</span><span>{item}</span></li>)}</ul></div><div className="rounded-2xl border border-papyrus-line bg-paper-card p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Metodo di lettura</p><h2 className="mt-2 font-serif text-2xl font-bold">Tre livelli, una sola pagina</h2><div className="mt-6 space-y-5"><div><strong>Essenziale</strong><p className="mt-1 text-sm leading-6 text-ink-soft">Sintesi e orientamento rapido.</p></div><div><strong>Studio</strong><p className="mt-1 text-sm leading-6 text-ink-soft">Struttura, contesto e cronologia.</p></div><div><strong>Critica</strong><p className="mt-1 text-sm leading-6 text-ink-soft">Livelli critici, motivazioni, testo e bibliografia.</p></div></div>{capitoli.length > 0 && <Link href={`/bibbia/${slug}/${firstChapter}`} className="mt-7 inline-flex min-h-11 items-center rounded-full border border-bronze px-4 py-2 text-sm text-bronze hover:bg-bronze hover:text-papyrus">Apri {abbr} {firstChapter} →</Link>}</div></aside></div></section>
  </main></AppShell>;
}
