  ...
  return <AppShell><main>
    ...
    <section id="studio" className="scroll-mt-36 mx-auto max-w-[1180px] px-5 py-12 md:px-8 md:py-16">
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Architettura letteraria</p>
          <h2 className="mt-2 font-serif text-3xl font-bold md:text-4xl">Struttura del libro</h2>
          <p className="mt-3 max-w-3xl text-base leading-7 text-ink-soft">Le macro-sezioni sono rappresentate in proporzione alla loro estensione nel libro. Ogni segmento apre direttamente il punto di ingresso della sezione.</p>
          <LiteraryArchitectureMap
            bookSlug={slug}
            bookAbbreviation={abbr}
            totalChapters={libro.capitoli ?? capitoli.length}
            sections={macro}
            fallback={textFromUnknown(libro.profiloLetterario?.strutturaGenerale, 'Macro-sezioni strutturate non ancora registrate per questo libro. La sezione Studio resta disponibile e usa soltanto i dati già presenti nel corpus.')}
          />
        </div>
