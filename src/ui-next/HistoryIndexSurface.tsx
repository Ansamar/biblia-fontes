...
export default function HistoryIndexSurface({ history, basePath = '/rebuild/historical-explorer', corpusPath = '/rebuild' }: { history: HistoryIndexView; basePath?: string; corpusPath?: string }) {
  ...
  return <main className="mx-auto max-w-[1320px] px-5 py-10 md:px-8 md:py-14">
    ...
    {mode === 'book' && <div className="mt-9 grid gap-9 lg:grid-cols-[190px_minmax(0,1fr)] lg:gap-10">
      <aside><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-ink-faint">Ordine canonico</p><p className="mt-2 font-serif text-3xl font-semibold">{history.datasets.length}</p><p className="mt-2 text-xs leading-5 text-ink-faint">Libri collegati alla rete storica.</p><Link href={corpusPath} className="mt-6 inline-flex text-xs text-ink-soft hover:text-ink">← Corpus biblico</Link></aside>
      <div className="border-t border-papyrus-line">{grouped.map(([category, fields]) => <section key={category} className="grid gap-5 border-b border-papyrus-line py-6 md:grid-cols-[160px_minmax(0,1fr)]"><header><h2 className="font-serif text-lg font-semibold">{category}</h2></header><div className="grid gap-x-6 gap-y-0 sm:grid-cols-2 xl:grid-cols-3">{fields.map((field) => <Link key={field.id} href={`${basePath}/${field.slug}`} className="group block border-b border-papyrus-line/70 py-3 transition hover:border-bronze/50"><span className="font-serif text-[1.05rem] font-semibold text-ink-soft transition group-hover:text-ink">{field.title}</span>{(field.start !== undefined || field.end !== undefined) && <span className="mt-1 block font-mono text-[9px] tracking-wide text-ink-faint">{field.start !== undefined ? formatYear(field.start) : '…'} — {field.end !== undefined ? formatYear(field.end) : '…'}</span>}</Link>)}</div></section>)}</div>
    </div>}
