...
export default function HistoryIndexSurface({ history }: { history: HistoryIndexView }) {
  ...
  return <main className="mx-auto max-w-[1320px] px-5 py-10 md:px-8 md:py-14">
    <header className="grid gap-8 border-b border-papyrus-line pb-9 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-end">
      <div><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-bronze">Storia</p><h1 className="mt-2 font-serif text-5xl font-semibold md:text-6xl">Interroga il tempo intorno al testo.</h1><p className="reading-text mt-5 max-w-[72ch] text-ink-soft">Puoi partire da un libro oppure da una data. In entrambi i casi Biblia Fontes mette in relazione luoghi, poteri, eventi, istituzioni, testi e trasmissione senza equiparare memoria biblica e attestazione storica.</p></div>
      <p className="border-l border-papyrus-line pl-6 text-xs leading-6 text-ink-faint">Ogni elemento conserva il proprio grado di affidabilità: attestato, probabile, discusso, memoria, comparandum o narrativo.</p>
    </header>
    ...
    {mode === 'book' && <div className="mt-9 grid gap-9 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-12">
      <aside><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-ink-faint">Libri nella rete storica</p><p className="mt-2 font-serif text-4xl font-semibold">{history.datasets.length}</p><p className="mt-2 text-xs leading-5 text-ink-faint">Punti di ingresso disponibili per esplorare la storia intorno al testo.</p><Link href="/rebuild" className="mt-6 inline-flex text-xs text-ink-soft hover:text-ink">← Corpus biblico</Link></aside>
      <div className="border-t border-papyrus-line">{grouped.map(([category, fields]) => <section key={category} className="grid gap-5 border-b border-papyrus-line py-7 md:grid-cols-[180px_minmax(0,1fr)]"><header><h2 className="font-serif text-xl font-semibold">{category}</h2></header><div className="grid gap-x-8 sm:grid-cols-2 xl:grid-cols-3">{fields.map((field) => <Link key={field.id} href={`/rebuild/historical-explorer/${field.slug}`} className="group border-b border-papyrus-line/70 py-3"><span className="flex items-baseline justify-between gap-4"><span className="font-serif text-lg text-ink-soft group-hover:text-ink">{field.title}</span><span className="text-[10px] text-ink-faint group-hover:text-bronze">Apri →</span></span>{(field.start !== undefined || field.end !== undefined) && <span className="mt-1 block font-mono text-[9px] text-ink-faint">{field.start !== undefined ? formatYear(field.start) : '…'} — {field.end !== undefined ? formatYear(field.end) : '…'}</span>}</Link>)}</div></section>)}</div>
    </div>}
