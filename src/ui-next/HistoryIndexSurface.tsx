...
export default function HistoryIndexSurface({ history }: { history: HistoryIndexView }) {
  ...
  return <main className="mx-auto max-w-[1320px] px-5 py-10 md:px-8 md:py-14">
    ...
    {mode === 'book' && <div className="mt-9 grid gap-9 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-12">
      <aside><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-ink-faint">Campi storici</p><p className="mt-2 font-serif text-4xl font-semibold">{history.datasets.length}</p><p className="mt-2 text-xs leading-5 text-ink-faint">Libri collegati alla rete storica disponibile.</p><Link href="/rebuild" className="mt-6 inline-flex text-xs text-ink-soft hover:text-ink">← Corpus biblico</Link></aside>
