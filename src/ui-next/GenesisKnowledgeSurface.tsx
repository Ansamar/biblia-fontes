  ...
  return <main className="mx-auto max-w-[1600px] px-4 py-6 md:px-6 md:py-8">
    <header className="border-b border-papyrus-line pb-7">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-bronze">Biblia Fontes 2.0 · prototipo conoscitivo</p>
          <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-ink-faint">{reference}</p>
          <h1 className="mt-1 font-serif text-4xl font-semibold leading-tight md:text-5xl">{chapter.title}</h1>
          <p className="mt-4 max-w-3xl font-serif text-xl leading-8 text-ink-soft">Creazione · ordine · essere umano · responsabilità</p>
        ...
        <div className="border-l border-papyrus-line pl-5">
          <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-ink-faint">Come leggere questa esperienza</p>
          <p className="mt-2 text-sm leading-6 text-ink-soft">Scrittura, mondo, umanità, tradizione e ricezione non sono sezioni separate: sono cinque accessi allo stesso capitolo.</p>
        </div>
      </div>

      <div className="mt-6 flex flex-wrap gap-2" aria-label="Relazioni principali di Genesi 1">
    ...
    <nav className="sticky top-[100px] z-30 -mx-4 mb-8 flex overflow-x-auto border-b border-papyrus-line bg-papyrus/95 px-4 backdrop-blur md:-mx-6 md:px-6" aria-label="Dimensioni della conoscenza">
    ...
    <section className="mb-10 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.6fr)]">
      <article className={`border p-5 md:p-7 ${relationTone(active)}`}>
        <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-bronze">{dimensionLabels[active]}</p>
      ...
      <aside className="border-y border-papyrus-line py-5 xl:border xl:p-5">
        ...
        <p className="mt-3 text-sm leading-6 text-ink-soft">Un capitolo biblico osservato simultaneamente come testo, prodotto storico-culturale, esperienza umana, tradizione trasmessa e testo ricevuto.</p>
