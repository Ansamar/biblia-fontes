...
const numericRange=(d:any)=>{if(!d||typeof d!=='object')return null;const a=typeof d.inizio==='number'?d.inizio:null;const b=typeof d.fine==='number'?d.fine:null;if(a==null&&b==null)return null;const start=a??b;const end=b??a;return start==null||end==null?null:{start,end};};
...
export default function GlobalTimelineExplorer({books}:{books:Book[]}){
 ...
 const pos=(n:number):number=>{if(!extent||extent.max===extent.min)return 50;return ((n-extent.min)/(extent.max-extent.min))*100;};
 return <div>
  <div className="flex flex-col gap-4 rounded-2xl border border-papyrus-line bg-paper-card p-4 md:flex-row md:items-center md:justify-between">
   <div className="flex flex-wrap gap-2"><button type="button" onClick={()=>setCategory('tutti')} className={`rounded-full px-4 py-2 text-sm ${category==='tutti'?'bg-ink text-papyrus':'border border-papyrus-line text-ink-soft'}`}>Tutti</button>{categories.map(c=><button type="button" key={c} onClick={()=>setCategory(c)} className={`rounded-full px-4 py-2 text-sm ${category===c?'bg-ink text-papyrus':'border border-papyrus-line text-ink-soft'}`}>{categoryLabel(c)}</button>)}</div>
   <div className="flex flex-col gap-2 sm:flex-row"><input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Filtra libri" placeholder="Filtra libri…" className="min-h-10 rounded-full border border-papyrus-line bg-papyrus px-4 text-sm outline-none focus:border-bronze"/><div className="flex rounded-full border border-papyrus-line p-1"><button type="button" onClick={()=>setView('timeline')} aria-pressed={view==='timeline'} className={`rounded-full px-3 py-1 text-xs ${view==='timeline'?'bg-ink text-papyrus':''}`}>Timeline</button><button type="button" onClick={()=>setView('table')} aria-pressed={view==='table'} className={`rounded-full px-3 py-1 text-xs ${view==='table'?'bg-ink text-papyrus':''}`}>Schede</button></div></div>
  </div>
