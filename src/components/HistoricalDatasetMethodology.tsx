    ...
    <details className="rounded-2xl border border-papyrus-line bg-paper-card/75 px-4 py-3 shadow-sm">
      <summary className="cursor-pointer list-none marker:hidden">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-bronze">Trasparenza dei dati storici</p>
          ...
          <div className="flex flex-wrap gap-2 text-[10px] text-ink-faint">
            ...
            <span className={`rounded-full border px-2.5 py-1 ${errors.length ? 'border-seal/40 text-seal' : warnings.length ? 'border-bronze/40 text-bronze' : 'border-papyrus-line text-ink-faint'}`}>{errors.length ? `${errors.length} errori` : warnings.length ? `${warnings.length} avvisi` : 'controlli superati'}</span>
          </div>
        </div>
      </summary>

      <div className="mt-4 border-t border-papyrus-line pt-4 text-xs leading-5 text-ink-faint">
        <p>I controlli verificano relazioni, intervalli temporali, riferimenti cartografici, provenienza e struttura dei riferimenti biblici. Non misurano la verità storica delle interpretazioni: questa resta espressa dallo stato epistemico e dalle fonti associate a ciascuna entità.</p>
