  ...
  return (
    <main>
      <section className="relative overflow-hidden border-b border-papyrus-line">
        ...
        <div className="relative mx-auto grid max-w-[1220px] items-center gap-12 px-5 py-20 md:px-8 md:py-28 lg:grid-cols-[1.15fr_.85fr]">
          <div>
              ...
              {query && (
                <div className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-xl border border-papyrus-line bg-paper-card shadow-xl" role="listbox" aria-label="Risultati di ricerca">
                  ...
                  {results.map((libro) => {
                    ...
                    return <Link key={libro.id} href={isGenesis ? '/bibbia/genesi' : '#bibbia'} className="flex items-center justify-between border-b border-papyrus-line/60 px-5 py-4 last:border-0 hover:bg-papyrus-deep/40"><span><span className="block text-[10px] uppercase tracking-widest text-bronze">Libro</span><strong className="font-serif text-xl">{libro.titolo}</strong></span><span className="text-sm text-ink-faint">{libro.capitoli ?? '—'} capitoli {isGenesis ? '→' : ''}</span></Link>;
                  })}
      ...
      <section id="esplora" className="mx-auto max-w-[1220px] px-5 py-16 md:px-8 md:py-20">
      ...
      <section id="bibbia" className="border-y border-papyrus-line bg-paper-card/35">
        <div className="mx-auto max-w-[1100px] px-5 py-16 md:px-8">
          ...
          <div className="divide-y divide-papyrus-line">{pentateuco.map((libro, index) => { const isGenesis = normalize(libro.titolo) === 'genesi'; return <div key={libro.id} className={`grid grid-cols-[3rem_1fr_auto] items-center gap-4 py-5 ${isGenesis ? 'group' : 'opacity-55'}`}><span className="font-mono text-xs text-ink-faint">{String(index + 1).padStart(2, '0')}</span><span><strong className="font-serif text-2xl group-hover:text-bronze">{isGenesis ? <Link href="/bibbia/genesi">{libro.titolo}</Link> : libro.titolo}</strong>{libro.titoloEbraico && <span className="ml-3 text-sm text-ink-faint">{libro.titoloEbraico}</span>}</span><span className="text-sm text-ink-faint">{libro.capitoli ?? '—'} cap. {isGenesis ? '→' : ''}</span></div>; })}</div>
