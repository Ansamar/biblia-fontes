  ...
  return (
    <section aria-labelledby="historical-explorer-shell-title" className="overflow-hidden rounded-3xl border border-papyrus-line bg-paper-card shadow-sm">
      <header className="border-b border-papyrus-line p-5 md:p-7">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-bronze">Esploratore storico di Biblia Fontes · architettura 0.7</p>
      ...
      <div className="border-b border-papyrus-line bg-papyrus/20 px-5 py-4 md:px-7">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2" aria-label="Livelli storici">{(Object.keys(layerLabels) as ExplorerLayer[]).map((layer) => { const active = layers.includes(layer); return <button key={layer} type="button" onClick={() => toggleLayer(layer)} aria-pressed={active} className={`rounded-full border px-3 py-2 text-xs transition ${active ? 'border-bronze bg-bronze text-white' : 'border-papyrus-line text-ink-soft hover:border-bronze'}`}>{layerLabels[layer]}</button>; })}</div>
        </div>
      </div>

      <div className="border-b border-papyrus-line bg-ink px-5 py-4 text-papyrus md:px-7">
        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
          <div><p className="font-mono text-[9px] uppercase tracking-[0.18em] text-papyrus/55">Scenario storico · {formatYear(year)}</p><h3 className="mt-1 font-serif text-2xl font-bold">{activeScenario?.title ?? 'Transizione storica'}</h3><p className="mt-2 max-w-4xl text-sm leading-6 text-papyrus/75">{activeScenario?.summary ?? 'L’anno selezionato cade fuori dagli scenari esplicitamente modellati per questo campo storico. L’esploratore mostra gli elementi pertinenti senza imporre una cornice storica non registrata.'}</p></div>
      ...
      <div className="grid xl:grid-cols-[minmax(0,1.55fr)_380px]">
        ...
        <aside className="border-t border-papyrus-line bg-papyrus/30 p-6 xl:border-l xl:border-t-0" aria-live="polite">{selected ? <><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-bronze">Scheda · {selected.type}</p><h3 className="mt-2 font-serif text-3xl font-bold">{selected.label}</h3><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full border border-papyrus-line bg-paper-card px-3 py-1 text-xs font-semibold text-ink-soft">{statusLabels[selected.epistemicStatus]}</span><span className="rounded-full border border-papyrus-line px-3 py-1 text-xs text-ink-faint">{temporalLabel(selected)}</span></div>{selected.spatial?.region ? <p className="mt-3 text-sm text-ink-faint">Spazio: {selected.spatial.region}</p> : null}<p className="mt-6 text-base leading-7 text-ink">{selected.summary}</p>{selected.biblicalRefs?.length ? <div className="mt-6 border-t border-papyrus-line pt-5"><p className="font-mono text-[10px] uppercase tracking-wider text-bronze">Nei testi</p><HistoricalBiblicalReferences references={selected.biblicalRefs} originBookSlug={originBookSlug} year={year} entityId={selected.id} /><p className="mt-3 text-xs leading-5 text-ink-faint">Aprendo un riferimento, Biblia Fontes conserva la scena storica corrente per permettere il ritorno allo stesso anno e alla stessa entità.</p></div> : null}{selected.relations.length ? <div className="mt-6 border-t border-papyrus-line pt-5"><p className="font-mono text-[10px] uppercase tracking-wider text-bronze">Relazioni</p><ul className="mt-2 space-y-2 text-sm leading-6 text-ink-soft">{selected.relations.map((relation) => <li key={`${relation.targetId}-${relation.kind}`}><button type="button" onClick={() => selectEntity(relation.targetId)} className="text-left hover:text-bronze">→ {relation.label}</button></li>)}</ul></div> : null}<div className="mt-6 border-t border-papyrus-line pt-5"><p className="font-mono text-[10px] uppercase tracking-wider text-bronze">Fonti e provenienza</p><p className="mt-2 mb-3 text-xs leading-5 text-ink-faint">La classificazione distingue la natura della fonte dalla valutazione epistemica dell’entità.</p><HistoricalProvenance sources={selected.sources} /></div></> : null}</aside>
      </div>
    </section>
  );
}
