...
export default function ChapterStudy({ chapter }: { chapter: any }) {
  ...
  return (
    <>
      <div className="mt-7">
        <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-bronze">Livello di lettura</p>
        <DepthSelector onChange={setDepth} />
      </div>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px]">
        <article className="min-w-0">
          <section id="testo" className="scroll-mt-24 border-y border-papyrus-line py-10">
            ...
            <div className="mt-8 rounded-xl border border-dashed border-papyrus-line bg-paper-card/50 p-8 text-center">
              <p className="font-serif text-xl text-ink">Il Reader funziona anche senza un testo biblico integrale.</p>
              <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-ink-soft">Per ora il riferimento è l'ancora del percorso di studio. Un'edizione autorizzata potrà essere collegata qui senza modificare l'architettura della pagina.</p>
            </div>
          </section>
          ...
          <section id="in-breve" className="scroll-mt-24 py-10">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-ink-soft">{chapter.sintesi || 'Sintesi didattica in preparazione.'}</p>
          </section>
          ...
          {depth !== 'essential' && (
            <section id="struttura" className="scroll-mt-24 border-t border-papyrus-line py-10">
              ...
              <p className="mt-5 whitespace-pre-line leading-7 text-ink-soft">{chapter.struttura || chapter.analisiLetteraria?.strutturaPoetica || 'La struttura dettagliata sarà visualizzata qui a partire dai dati del capitolo.'}</p>
              <div id="contesto" className="mt-8 grid scroll-mt-24 gap-6 md:grid-cols-2">
                <div>
                  <h3 className="font-serif text-xl font-bold">Contesto storico-culturale</h3>
                  <p className="mt-2 text-sm leading-6 text-ink-soft">{chapter.contestoStorico || 'Distinguere sempre il mondo rappresentato dal racconto dal contesto culturale e dalla storia della composizione.'}</p>
                </div>
                <div>
                  <h3 className="font-serif text-xl font-bold">Tradizione</h3>
                  <p className="mt-2 text-sm leading-6 text-ink-soft">{chapter.tradizione || 'Il rapporto con tradizioni precedenti viene descritto solo quando i dati lo permettono.'}</p>
                </div>
              </div>
            </section>
          )}
          ...
          {depth !== 'essential' && (
            <section id="cronologia" className="scroll-mt-24 border-t border-papyrus-line py-10">
              ...
              <div className="mt-7 grid gap-5 md:grid-cols-3">
                <div className="border-t-2 border-ink pt-4"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-faint">Mondo narrato</p><p className="mt-2 text-sm leading-6 text-ink-soft">{chapter.numero === 1 ? 'Tempo primordiale: non è una data storica misurabile.' : 'Periodo o scena rappresentata dal racconto.'}</p></div>
                <div className="border-t-2 border-bronze pt-4"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-faint">Contesto storico</p><p className="mt-2 text-sm leading-6 text-ink-soft">{chapter.contestoStorico || 'Eventi e ambienti storici pertinenti saranno collegati come livello distinto.'}</p></div>
                <div className="border-t-2 border-seal pt-4"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-faint">Formazione del testo</p><p className="mt-2 text-sm leading-6 text-ink-soft">{timelineLabel}</p></div>
              </div>
              {chapter.datazione?.nota && <p className="mt-5 text-xs leading-5 text-ink-faint">{chapter.datazione.nota}</p>}
            </section>
          )}
          ...
          {depth === 'critical' && (
            <section id="livelli-critici" className="scroll-mt-24 border-t border-papyrus-line py-10">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-soft">Qui distinguiamo dati testuali, tradizioni, modelli ricostruttivi e redazione. La presenza di un livello non implica automaticamente che sia una fonte documentaria certa.</p>
              <div className="mt-7 divide-y divide-papyrus-line border-y border-papyrus-line">
                {layers.length ? layers.map((layer:any, i:number) => (
                  <div key={layer._key || i} className="py-6">
                    ...
                    {layer.fonte?.nome && layer.fonte?.sigla && <p className="mt-1 text-sm font-medium text-ink">{layer.fonte.nome}</p>}
                    <p className="mt-2 text-sm leading-6 text-ink-soft">{layer.descrizione || layer.motivazione || layer.fonte?.descrizione || 'Attribuzione registrata nel modello critico.'}</p>
                    {layer.motivazione && layer.descrizione && <details className="mt-3"><summary className="cursor-pointer text-sm text-bronze">Perché questa attribuzione?</summary><p className="mt-2 text-sm leading-6 text-ink-soft">{layer.motivazione}</p></details>}
                  </div>
                )) : <p className="py-6 text-sm text-ink-soft">Nessuna attribuzione critica strutturata disponibile per questo capitolo.</p>}
              </div>
            </section>
          )}
          ...
          <section id="bibliografia" className="scroll-mt-24 border-t border-papyrus-line py-10">
            ...
            {bibliography.length ? <div className="mt-6 divide-y divide-papyrus-line border-y border-papyrus-line">{bibliography.map((item:any, i:number) => <div key={item._key || i} className="py-4"><p className="text-sm leading-6 text-ink">{item.citazione || item.titolo || String(item)}</p>{item.tipo && <p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-ink-faint">{item.tipo}</p>}</div>)}</div> : <p className="mt-5 text-sm leading-6 text-ink-soft">Bibliografia specifica del capitolo non ancora disponibile.</p>}
          </section>
        </article>
