...
export default function UniversalChapterStudy({ chapter, reference, biblicalText }: { chapter: any; reference: string; worldNarratedLabel?: string; biblicalText?: BiblicalTextUnit | null }) {
  const [depth, setDepth] = useState<StudyDepth>('study');
  const [sourceContextId, setSourceContextId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fromSource = params.get('source') === 'source';
    const sourceId = fromSource ? params.get('entity') : null;
    if (sourceId) {
      setSourceContextId(sourceId);
      setDepth('critical');
      window.requestAnimationFrame(() => document.getElementById('livelli-critici')?.scrollIntoView({ block: 'start' }));
      return;
    }
  ...
  const sourceLayer = sourceContextId ? layers.find((layer: any) => layer?.fonte?._id === sourceContextId) : null;

  return <>
    {sourceContextId && <div className="mt-7 rounded-2xl border border-bronze/45 bg-bronze/8 p-4 md:flex md:items-center md:justify-between md:gap-5"><div><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Contesto conservato · Fonte</p><p className="mt-1 text-sm leading-6 text-ink-soft">Sei arrivato a {reference} dalla mappa delle fonti. L’apparato critico è stato aperto automaticamente{sourceLayer?.fonte?.sigla ? ` sulla fonte ${sourceLayer.fonte.sigla}` : ''}.</p></div>{sourceLayer?.fonte?.nome && <strong className="mt-3 block shrink-0 font-serif text-lg text-ink md:mt-0">{sourceLayer.fonte.nome}</strong>}</div>}
    ...
    <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px]">
      <article className="min-w-0">
        ...
        {depth !== 'essential' && <section id="struttura" className="scroll-mt-36 border-t border-papyrus-line py-12"><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Studio</p><h2 className="mt-2 font-serif text-3xl font-bold">Struttura, contesto e formazione</h2><p className="reading-text mt-5 whitespace-pre-line text-ink-soft">{structureText}</p><div id="contesto" className="mt-10 grid scroll-mt-36 gap-5 md:grid-cols-2"><div className="rounded-xl border border-papyrus-line bg-paper-card/50 p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Mondo dietro il testo</p><h3 className="mt-2 font-serif text-xl font-bold">Contesto storico-culturale</h3><p className="mt-3 leading-7 text-ink-soft">{contextText}</p></div><div className="rounded-xl border border-papyrus-line bg-paper-card/50 p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Formazione</p><h3 className="mt-2 font-serif text-xl font-bold">Tradizione / redazione</h3><p className="mt-3 leading-7 text-ink-soft">{traditionText}</p></div></div></section>}

        {depth === 'critical' && <section id="livelli-critici" className="scroll-mt-36 border-t border-papyrus-line py-12"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-widest text-bronze">Critica</p><h2 className="mt-2 font-serif text-3xl font-bold">Livelli critici / attribuzioni</h2></div><p className="text-sm text-ink-faint">{layers.length} livelli registrati</p></div>{criticalText && <p className="reading-text mt-5 text-ink-soft">{criticalText}</p>}<div className="mt-8 space-y-4">{layers.length ? layers.map((layer:any, i:number) => { const contextual = sourceContextId && layer?.fonte?._id === sourceContextId; return <article key={layer._key || i} className={`rounded-2xl border p-5 shadow-sm md:p-6 ${contextual ? 'border-bronze bg-bronze/8 ring-1 ring-bronze/20' : 'border-papyrus-line bg-paper-card/55'}`}><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-start gap-3"><span className="mt-1 font-mono text-lg text-bronze" aria-hidden="true">{certaintySymbol(layer.certezza)}</span><div><div className="flex flex-wrap items-center gap-2"><strong className="font-serif text-2xl">{layer.fonte?.sigla || layer.fonte?.nome || 'Livello critico'}</strong>{contextual && <span className="rounded-full bg-bronze px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white">Fonte selezionata</span>}{(layer.versettoInizio != null || layer.versettoFine != null) && <span className="font-mono text-[11px] text-ink-faint">vv. {layer.versettoInizio ?? '…'}–{layer.versettoFine ?? '…'}</span>}</div>{layer.fonte?.nome && layer.fonte?.sigla && <p className="mt-1 text-sm font-medium text-ink">{layer.fonte.nome}</p>}</div></div><span className="rounded-full border border-papyrus-line px-3 py-1 text-[11px] text-ink-soft">{certaintyLabel(layer.certezza)}</span></div><p className="mt-4 leading-7 text-ink-soft">{textFromUnknown(layer.descrizione) || textFromUnknown(layer.motivazione) || textFromUnknown(layer.fonte?.descrizione) || 'Attribuzione registrata nel modello critico.'}</p>{layer.motivazione && layer.descrizione && <details className="mt-4 border-t border-papyrus-line pt-4"><summary className="cursor-pointer font-medium text-bronze">Perché questa attribuzione?</summary><p className="mt-3 leading-7 text-ink-soft">{textFromUnknown(layer.motivazione)}</p></details>}</article>; }) : <p className="rounded-xl border border-papyrus-line p-5 text-ink-soft">Nessuna attribuzione critica strutturata disponibile.</p>}</div></section>}
