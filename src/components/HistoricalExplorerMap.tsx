...
export default function HistoricalExplorerMap({ entities, areas = [], selectedId, year, onSelect }: {
  ...
  const points: HistoricalMapPoint[] = entities.flatMap((entity) => {
    ...
    return [{
      ...
      subtitle: entity.spatial?.region,
    }];
  });

  const activeAreaRecords = areas.filter((area) => area.temporal.start <= year && area.temporal.end >= year);
  const mapAreas: HistoricalMapArea[] = activeAreaRecords.map((area) => ({
    id: area.id,
  ...
  return <div>
    <HistoricalMap
      ...
      onSelect={onSelect}
      headerRight={<span>{points.filter((point) => point.active !== false).length} elementi attivi · {points.length} georeferenziati</span>}
      footer={areas.length > 0
        ? 'Base geografica comune di Biblia Fontes. Le aree tratteggiate sono ricostruzioni didattiche approssimate e non vanno lette come frontiere storiche certe.'
        : 'Base geografica comune di Biblia Fontes. I marker derivano dalle coordinate archiviate nel dataset.'}
    />

    {activeAreaRecords.length > 0 ? (
      <details className="border-x border-b border-papyrus-line bg-paper-card px-4 py-3">
